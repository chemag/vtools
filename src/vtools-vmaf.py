...
def run_vmaf(distorted, reference, outfile, debug):
    ...
    command = f"ffmpeg -i {distorted} -i {reference} -lavfi libvmaf=\"model=path=/usr/share/model/vmaf_v0.6.1neg.json:feature=name=psnr:log_fmt=json:log_path={vmaf_file}:n_threads=32\" -f null -"
